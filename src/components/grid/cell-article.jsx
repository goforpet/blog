...
export default function CellArticle({ post }) {
  return (
    <article className="cell-article" itemScope itemType="http://schema.org/Article">
      <Link to={`/${post.slug}`}>
        <PostThumb post={post} />
      </Link>
      <div className="post-body">
        ...
        <div>{post.excerpt && <div className="excerpt">{post.excerpt}</div>}</div>
        <div className="post-link">
          <Link to={`/${post.slug}`} aria-label={`Leggi "${post.title}"`}>
            <i className="icon-goforpet-plus" /> Leggi
          </Link>
        </div>
      </div>
    </article>
